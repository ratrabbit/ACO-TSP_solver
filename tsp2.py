...
class Hormiga:
	...
	def __init__(self, id_h):
		self.hormiga_id = id_h
		self.vertices_recorridos = []
		self.ruta_actual_distancia = 0
		self.forward_mode = True
		self.rutas_generadas = []
		self.nodo_actual = 0
		self.nodo_inicial = 0
		self.probas = [[]]
		self.ciudadesRestantes = []

	def vertice_visitado(self,v):
		if v in self.vertices_recorridos:
			ultimo = self.vertices_recorridos[-1]
			while ultimo != v:
				self.vertices_recorridos.pop()
				#if len(self.vertices_recorridos) > 0:
				ultimo = self.vertices_recorridos[-1]
			...
			return False
def tsp(grafica,iteraciones,coef_evaporacion,delta_feromona,alfa=1,beta=1):
	hormigas = []
	numhormigas = len(grafica)
	matriz_feromona = [[1.0 for x in range(numhormigas)] for y in range(numhormigas)]
	#print matriz_feromona
	for num_h in range(numhormigas):
		hormigas.append(Hormiga(num_h))
	a = range(numhormigas)
	random.shuffle(a)
	for h in hormigas:
		h.nodo_inicial = a.pop()
	for i in range(iteraciones):
		for h in hormigas:
			h.vertices_recorridos = []
			h.ciudadesRestantes = range(numhormigas)
			h.probas = [[10000.0 for x in range(numhormigas)] for y in range(numhormigas)]
			h.ruta_actual_distancia = 0
			h.vertices_recorridos.append(h.nodo_inicial)
			h.nodo_actual = h.nodo_inicial
			h.ciudadesRestantes.remove(h.nodo_actual)
			while len(h.vertices_recorridos) < numhormigas:
				suma_feromona_vecinos = 0.0
				for ciudad in h.ciudadesRestantes:
					#if not ciudad in h.vertices_recorridos:
					suma_feromona_vecinos += (matriz_feromona[h.nodo_actual][ciudad]** alfa)+(1.0/grafica[h.nodo_actual][ciudad]**beta)
				for ciudad in h.ciudadesRestantes:
					#if not ciudad in h.vertices_recorridos: 
					h.probas[h.nodo_actual][ciudad] = (matriz_feromona[h.nodo_actual][ciudad]**alfa + (1.0/grafica[h.nodo_actual][ciudad]**beta))/suma_feromona_vecinos
					h.probas[ciudad][h.nodo_actual] = h.probas[h.nodo_actual][ciudad]
					#else:
					#h.probas[h.nodo_actual][ciudad] = 10000
					#h.probas[ciudad][h.nodo_actual] = h.probas[h.nodo_actual][ciudad]
				...
				for ciudad in h.ciudadesRestantes:
					#if not ciudad in h.vertices_recorridos:
					suma_proba += h.probas[h.nodo_actual][ciudad]
					if suma_proba >= aleatorio:
						h.vertices_recorridos.append(ciudad)
						h.nodo_actual = ciudad
						break
						#else:
						#h.nodo_actual = h.vertices_recorridos[-1]
				h.ciudadesRestantes.remove(h.nodo_actual)
				for ind in range(numhormigas):
					for jay in range(numhormigas):
						matriz_feromona[ind][jay] *= (1.0 - coef_evaporacion)
			h.vertices_recorridos.append(h.nodo_inicial)
			h.forward_mode = False

			for dis in range(len(h.vertices_recorridos)-1):
				h.ruta_actual_distancia += grafica[h.vertices_recorridos[dis]][h.vertices_recorridos[dis+1]]
				matriz_feromona[h.vertices_recorridos[dis]][h.vertices_recorridos[dis+1]] += delta_feromona
				for ind in range(numhormigas):
					for jay in range(numhormigas):
						matriz_feromona[ind][jay] *= (1.0 - coef_evaporacion)

			h.rutas_generadas.append([h.ruta_actual_distancia,h.vertices_recorridos,i])
			h.vertices_recorridos = []
			h.ruta_actual_distancia = 0

	result = hormigas[0].rutas_generadas[0]
	for h in hormigas:
		for routes in h.rutas_generadas:
			#print routes
			if routes[0] < result[0]:
				result = routes
	return result
...
G =[[0,107,241,190,124,80,316,76,152,157,283,133,113,297,228,129,348,276,188,150,65,341,184,67,221,169,108,45,167],
